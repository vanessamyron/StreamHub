...
async function getStreamerTwitch() {
	...
	.then((user) =>	console.log('Success:', user));
		...
		if(obj === undefined) {
			...
			addStreamer(obj.type, obj.user_name, obj.title ,obj.viewer_count, obj.follows);
			/*
			WOrking on later -
...
async function getStreamerMixer() {
	...
	fetch(BASE_URL_MIXER + user, {
		...
		.then((user) => {
			...
			if(user.online === true) {
				let followers = user.stream.channel.follows;
				addStreamer(user.online, user.token, user.title ,user.viewersCurrent, followers);
...
function addStreamer(status, name, title, viewers, followers){
...
$.ajax({
  ...
  var total = data._total - 1;
  var follows = data.follows;
  for (var start = 0; start < total; start++) {
    var userInfo = follows[start].channel;
    var addHtml = "<div class='col-xs-12 row'><a href='" + userInfo.url + "'>";
    addHtml += "<div class='col-xs-2'><img class='img-responsive' src='" + userInfo.logo + "'></div>";
    addHtml += "<div class='col-xs-10'><h4>" + userInfo.display_name + "<div class=" + userInfo.name + "></div>";
    addHtml += "<small>" + userInfo.status + "</small></h4></div>";
    addHtml += "</a></div>";
    $(".appendData").append(addHtml);
    
    followersCount.push(userInfo.name);
...
function getLive() {
  for (var i = 0; i < followersCount.length; i++) {
    var channelUrl = "https://api.twitch.tv/kraken/streams/" + followersCount[i];
    $.ajax({
      dataType: "jsonp",
      url: channelUrl,
      async: false,
      ...
      if (newObject.stream) {
        ...
        var getLive = "<a href=" + newObj.stream.channel.url + "><div class='col-xs-4'>";
        getLive+= "<img src=" + newObj.stream.preview.medium + " class='img-responsive'>";
        getLive += "<h4>" + newObj.stream.channel.display_name + "<br><small>playing " + newObj.stream.channel.game;
        getLive += "</a></small></h4></div>";
        
        $(".viewLive").append(getLive);
      }
    });  
  }
